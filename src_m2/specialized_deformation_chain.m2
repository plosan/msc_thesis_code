...
for idx from 0 to (length primeList - 1) do (
    ...
    filename := concatenate("specialized_", toString(a), "_", toString(b), "_p", toString(p), ".txt");
    ...
    R := GF(p)[x, y, t1, t2];
    f := x^4 + y^7;
    for t1 from 0 to p - 1 do (
        for t2 from 0 to p - 1 do (
            -- Compute deformation
            delta = t1 * x^2 * y^4 + t2 * x^2 * y^5;
            g = f + delta;
            isIrred = isPrime(g);

            -- Parametric r chain
            out = rChainVerbose(g, 1);
            nuInv := out_0;
            fRoots := out_1;
            ...
            fileAppend << "f = " << toString(f) << endl;
            fileAppend << "g = " << toString(g) << endl;
            fileAppend << "t1 = " << toString(t1) << ", t2 = " << toString(t2) << endl;
